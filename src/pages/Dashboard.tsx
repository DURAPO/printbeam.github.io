      ...
      <header className="border-b border-border/60 bg-card/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="mx-auto max-w-5xl flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-2.5">
            <div className="flex size-7 items-center justify-center rounded bg-success/15 text-success">
              <Printer className="size-3.5" />
            </div>
            <span className="text-xs font-bold tracking-tight">cloud_print_pixie</span>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-[11px] text-success font-medium mb-0.5">
                // dashboard
              </p>
            ...
            {jobs === undefined ? (
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  No print jobs yet. Start your first one!
                </p>
                <Button
                  onClick={() => navigate("/new-print")}
                  variant="outline"
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-border">
                        ...
                        <TableHead className="text-[11px] text-muted-foreground">Store</TableHead>
                        ...
                        return (
                          <TableRow key={job._id} className="border-border">
                            ...
                            <TableCell className="text-xs text-muted-foreground">
                              <span className="truncate max-w-[140px] block">{job.storeName}</span>
                            </TableCell>
                            <TableCell className="text-[11px] text-muted-foreground">
                              {job.copies}× · {job.color ? "color" : "B&W"} · {job.paperSize.split(" ")[0]}
                            </TableCell>
                            <TableCell>
                              <Badge
                                variant="outline"
                              ...
                              >
                                <StatusIcon className={`size-2.5 ${job.status === "processing" ? "animate-spin" : ""}`} />
                                {sc.label}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-[11px] text-muted-foreground">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-muted-foreground/70">
                            {job.copies}× · {job.color ? "color" : "B&W"} · {formatDate(job.createdAt)}
                          </span>
                          {(job.status === "pending" || job.status === "processing") && (
                            <button
                              onClick={() => handleCancel(job._id)}
