...
const capabilities = [
  ...
  { icon: Shield, title: "Enterprise security", description: "Files encrypted in transit, never stored longer than needed, accessible only to you." },
];
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-background font-mono">
      {/* Nav */}
      <nav className="border-b border-border bg-card/60 glass sticky top-0 z-50">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 py-3.5">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <ThemeSwitcher />
            <button onClick={() => navigate("/auth")} className="text-xs text-muted-foreground hover:text-foreground transition-colors px-3 py-1.5">
              Sign in
            </button>
            <button onClick={() => navigate("/auth")} className="rounded-lg bg-[var(--ring)] text-white px-4 py-2 text-xs font-medium hover:bg-[var(--ring)]/90 transition-colors">
              Get started
            </button>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden border-b border-border">
        <div className="absolute inset-0 bg-gradient-to-b from-[var(--ring)]/5 to-transparent pointer-events-none" />
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16 sm:py-20 md:py-28 relative">
          <motion.div initial="hidden" animate="visible" className="max-w-2xl">
            <motion.div variants={fadeUp} custom={0} className="inline-flex items-center gap-2 rounded-full border border-[var(--ring)]/20 bg-[var(--ring)]/5 px-3 py-1 text-[11px] text-[var(--ring)] mb-6 font-medium tracking-wide">
              <span className="size-1.5 rounded-full bg-[var(--ring)]" />
            ...
            <motion.div variants={fadeUp} custom={3} className="mt-8 flex flex-wrap gap-3">
              <button onClick={() => navigate("/auth")} className="inline-flex items-center gap-2 rounded-lg bg-[var(--ring)] text-white px-5 py-2.5 text-sm font-medium hover:bg-[var(--ring)]/90 transition-colors">
                Start printing <ArrowRight className="size-4" />
              </button>
              <button onClick={() => navigate("/store-onboarding")} className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-5 py-2.5 text-sm font-medium hover:bg-muted transition-colors">
        ...
        <div className="absolute right-8 top-16 hidden lg:block text-[var(--ring)]/10 select-none">
...
{`$ printbeam submit \\
  ...
  ✓ queued · ready by 2:15pm`}
      ...
      <section className="border-b border-border bg-surface/50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6 grid grid-cols-3 gap-4 sm:gap-6">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
              <p className="text-lg md:text-xl font-bold text-[var(--ring)]">{stat.value}</p>
      ...
      <section className="border-b border-border">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16 md:py-20">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mb-10">
            <p className="text-[11px] text-[var(--ring)] font-medium mb-2 tracking-wide">// capabilities</p>
            <h2 className="text-xl md:text-2xl font-bold tracking-tight">Built for speed and reliability.</h2>
          </motion.div>
          <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2">
            {capabilities.map((cap, i) => (
              <motion.div key={cap.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="rounded-xl border border-border bg-card p-6 transition-colors hover:border-border/80">
                <div className="flex items-center gap-3 mb-3">
                  <div className="flex size-9 items-center justify-center rounded-lg bg-[var(--ring)]/10 text-[var(--ring)] border border-[var(--ring)]/15"><cap.icon className="size-4" /></div>
      ...
      <section className="border-b border-border">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16 md:py-20">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mb-10">
            <p className="text-[11px] text-[var(--ring)] font-medium mb-2 tracking-wide">// how it works</p>
            <h2 className="text-xl md:text-2xl font-bold tracking-tight">Two flows. Both simple.</h2>
          </motion.div>
          <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-xl border border-border bg-card p-6">
              <div className="flex items-center gap-2 mb-4">
                <div className="size-8 rounded-lg bg-[var(--ring)]/10 flex items-center justify-center text-[var(--ring)] border border-[var(--ring)]/20"><Zap className="size-4" /></div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.step} className="flex items-center gap-3">
                    <span className="text-[10px] text-[var(--ring)] font-medium tracking-wider w-6">{item.step}</span>
                    <span className="text-xs text-muted-foreground">{item.label}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="rounded-xl border border-border bg-card p-6">
              <div className="flex items-center gap-2 mb-4">
                <div className="size-8 rounded-lg bg-[var(--ring)]/10 flex items-center justify-center text-[var(--ring)] border border-[var(--ring)]/20"><Printer className="size-4" /></div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16 md:py-24 text-center">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              <button onClick={() => navigate("/auth")} className="inline-flex items-center gap-2 rounded-lg bg-[var(--ring)] text-white px-6 py-2.5 text-sm font-medium hover:bg-[var(--ring)]/90 transition-colors">
                Start printing <ArrowRight className="size-4" />
              </button>
              <button onClick={() => navigate("/store-onboarding")} className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-6 py-2.5 text-sm font-medium hover:bg-muted transition-colors">
                <Store className="size-4" /> Register your store
              </button>
            </div>
          </motion.div>
          ...
          <div className="grid gap-8 sm:grid-cols-4 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Zap className="size-3 text-[var(--ring)]" /><span className="text-xs font-bold">PrintBeam</span>
              </div>
